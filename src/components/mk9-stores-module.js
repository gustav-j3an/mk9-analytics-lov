...
import { Plus, Search, Store, MapPin, Building2, CheckCircle2 } from "lucide-react";
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-card/50 p-4 rounded-xl border border-border/50 backdrop-blur-sm">
        <div className="relative w-full md:max-w-sm">
          ...
          <Input placeholder="Buscar por nome, rede, cidade ou UF..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9"/>
        </div>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (Array.from({ length: 5 }).map((_, i) => (<TableRow key={i}>
                ...
                <TableCell colSpan={4} className="h-32 text-center text-muted-foreground">
              ...
              </TableRow>) : (filtered.map((s) => (<TableRow key={s.id} className="group transition-colors">
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex flex-col gap-1">
                      ...
                      {s.channel && (<div className="text-xs text-muted-foreground">{s.channel}</div>)}
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-1.5 text-sm">
