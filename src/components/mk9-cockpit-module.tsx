...
import type { Mk9CockpitOverview, Mk9HealthLevel, Mk9QuickActionId } from "@/lib/mk9-cockpit/types";
...
export function Mk9CockpitModule({
  ...
  if (q.isError)
    return <Mk9ErrorState message={(q.error as Error)?.message} onRetry={() => q.refetch()} />;
  if (!data) return null;
    ...
    <div className="space-y-8 animate-fade-up">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <Mk9MetricCard
          ...
          icon={CheckCircle2}
          label="Realizadas"
          value={data.kpis.realizadas}
          hint={`${data.kpis.coberturaPct}% cobertura`}
        />
        <Mk9MetricCard
          color="amber"
          icon={AlertTriangle}
        ...
        <Mk9MetricCard
          ...
          icon={AlertTriangle}
          label="Críticas"
          value={data.health.blockingIssues}
          hint="Bloqueantes"
        />
        <Mk9MetricCard
          color="purple"
          icon={TrendingUp}
          label="Cobertura"
          value={`${data.kpis.coberturaPct}%`}
          hint={`Ritmo: ${data.health.pacePercentage}%`}
        />
        <Mk9MetricCard
          color="blue"
          icon={Users}
              ...
              >
                <div
                  className={cn(
                    ...
                    data.health.level === "SAUDAVEL" ? "bg-emerald-500" : "bg-amber-500",
                  )}
                />
