...
import { Plus, Search, UserCircle, MapPin, Smartphone, CheckCircle2 } from "lucide-react";
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-card/50 p-4 rounded-xl border border-border/50 backdrop-blur-sm">
        <div className="relative w-full md:max-w-sm">
          ...
          <Input placeholder="Buscar por nome, cidade ou UF..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9"/>
        </div>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (Array.from({ length: 5 }).map((_, i) => (<TableRow key={i}>
              ...
              </TableRow>) : (filtered.map((p) => (<TableRow key={p.id} className="group transition-colors">
                  <TableCell>
