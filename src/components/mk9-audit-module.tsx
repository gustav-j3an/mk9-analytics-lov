    ...
    <div className="space-y-8 animate-fade-up">
      ...
      <Mk9Panel>
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-[10px] font-black uppercase tracking-[0.25em] text-slate-500">Parâmetros de Auditoria</h3>
          <div className="flex items-center gap-2">
            <Button size="sm" variant="ghost" className="h-8 text-slate-400 hover:text-white hover:bg-white/5" onClick={() => { industryQ.refetch(); storeQ.refetch(); promoterQ.refetch(); }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <Mk9MetricCard label="Contratadas" value={globalTotals.contratadas} icon={RouteIcon} color="blue" />
        <Mk9MetricCard label="Realizadas" value={globalTotals.realizadas} icon={FileCheck} color="emerald" />
        <Mk9MetricCard label="Pendentes" value={globalTotals.pendentes} icon={AlertTriangle} color="amber" />
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <Tabs value={tab} onValueChange={(v) => setTab(v as TabKey)} className="w-auto">
            <TabsList className="bg-command-deep border border-white/5 p-1">
              <TabsTrigger value="industry" className="data-[state=active]:bg-command-purple data-[state=active]:text-white uppercase text-[10px] font-black tracking-widest">Por Indústria</TabsTrigger>
              <TabsTrigger value="store" className="data-[state=active]:bg-command-purple data-[state=active]:text-white uppercase text-[10px] font-black tracking-widest">Por Loja</TabsTrigger>
              <TabsTrigger value="promoter" className="data-[state=active]:bg-command-purple data-[state=active]:text-white uppercase text-[10px] font-black tracking-widest">Por Promotor</TabsTrigger>
              <TabsTrigger value="review" className="data-[state=active]:bg-command-purple data-[state=active]:text-white uppercase text-[10px] font-black tracking-widest">Revisão Manual</TabsTrigger>
            </TabsList>
          </Tabs>
          ...
          {tab !== "review" && (
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" className="h-9 border-white/10 bg-white/5 text-slate-400 hover:text-white">
    ...
    <Table>
      <TableHeader className="bg-white/5">
        <TableRow className="border-white/5 hover:bg-transparent">
          ...
          <TableHead className="text-[10px] font-black uppercase tracking-widest text-slate-500">Período</TableHead>
          <TableHead className="text-right text-[10px] font-black uppercase tracking-widest text-slate-500">Contratadas</TableHead>
          <TableHead className="text-right text-[10px] font-black uppercase tracking-widest text-slate-500">Realizadas</TableHead>
          <TableHead className="text-right text-[10px] font-black uppercase tracking-widest text-slate-500">Cobertura</TableHead>
        </TableRow>
      </TableHeader>
      <TableBody>
        {rows.map((r) => (
          <TableRow key={r.industryId} className="border-white/5 hover:bg-white/[0.02] transition-colors">
            ...
            <TableCell className="text-right font-medium text-emerald-400">{r.realizadas}</TableCell>
            <TableCell className="text-right">
              <Mk9Badge variant={r.coberturaPct >= 100 ? "success" : r.coberturaPct >= 80 ? "info" : "warning"}>
                {r.coberturaPct}%
              </Mk9Badge>
            </TableCell>
          </TableRow>
...
function StoreTable({ q, search }: { q: any, search: string }) {
  ...
  if (search) {
    ...
    rows = rows.filter(r => r.storeName.toLowerCase().includes(s) || r.industryName.toLowerCase().includes(s));
  }
  if (rows.length === 0) return <Mk9EmptyState />;
  ...
  return (
    <Table>
      <TableHeader className="bg-white/5">
        <TableRow className="border-white/5 hover:bg-transparent">
          <TableHead className="text-[10px] font-black uppercase tracking-widest text-slate-500">Loja / Rede</TableHead>
          <TableHead className="text-[10px] font-black uppercase tracking-widest text-slate-500">Indústria</TableHead>
          <TableHead className="text-right text-[10px] font-black uppercase tracking-widest text-slate-500">Contratadas</TableHead>
          <TableHead className="text-right text-[10px] font-black uppercase tracking-widest text-slate-500">Realizadas</TableHead>
      ...
      <TableBody>
        {rows.map((r) => (
          <TableRow key={`${r.industryId}-${r.storeId}`} className="border-white/5 hover:bg-white/[0.02] transition-colors">
            <TableCell>
              <div className="font-bold text-white uppercase tracking-tight">{r.storeName}</div>
              <div className="text-[9px] text-slate-500 font-black uppercase tracking-widest">{r.chain ?? "—"}</div>
            </TableCell>
            <TableCell className="text-[10px] font-bold text-slate-400">{r.industryName}</TableCell>
            <TableCell className="text-right font-medium text-slate-300">{r.contratadas}</TableCell>
            <TableCell className="text-right font-medium text-emerald-400">{r.realizadas}</TableCell>
            <TableCell>
               <Mk9Badge variant={r.status === "COMPLETO" ? "success" : r.status === "PARCIAL" ? "warning" : "danger"}>
                {r.status}
              </Mk9Badge>
            </TableCell>
          </TableRow>
    ...
    <Table>
      <TableHeader className="bg-white/5">
        <TableRow className="border-white/5 hover:bg-transparent">
          ...
          <TableHead className="text-right text-[10px] font-black uppercase tracking-widest text-slate-500">Lojas</TableHead>
          <TableHead className="text-right text-[10px] font-black uppercase tracking-widest text-slate-500">Contratadas</TableHead>
          <TableHead className="text-right text-[10px] font-black uppercase tracking-widest text-slate-500">Realizadas</TableHead>
          <TableHead className="text-right text-[10px] font-black uppercase tracking-widest text-slate-500">Cobertura</TableHead>
        </TableRow>
      ...
      <TableBody>
        {rows.map((r) => (
          <TableRow key={r.promoterId ?? "none"} className="border-white/5 hover:bg-white/[0.02] transition-colors">
