...
import { BarChart3, Calendar, ChevronsLeft, ChevronsRight, ClipboardCheck, Factory, Loader2, LogOut, PackageCheck, Route, ShieldAlert, ShieldCheck, Store, Upload, Users, } from "lucide-react";
...
export function Mk9AnalyticsApp() {
    ...
    return (<main className="min-h-screen w-full bg-[#f8fafc] dark:bg-[#020617] text-foreground flex overflow-hidden font-sans">
      {/* Sidebar */}
      <aside className={cn("bg-white dark:bg-[#0f172a] border-r transition-all duration-300 flex flex-col glass-panel shrink-0 z-20", collapsed ? "w-20" : "w-64")}>
        ...
        <div className="flex-1 overflow-y-auto py-6">
          <nav className="px-3 space-y-1">
            <SidebarItem icon={BarChart3} label="Dashboard" active={activeModule === "dashboard"} onClick={() => setActiveModule("dashboard")}/>
            <SidebarItem icon={Gauge} label="Cockpit" active={activeModule === "cockpit"} onClick={() => setActiveModule("cockpit")}/>
            <SidebarItem icon={ClipboardCheck} label="Conciliação" active={activeModule === "conciliacao"} onClick={() => setActiveModule("conciliacao")}/>
            <SidebarItem icon={PackageCheck} label="Qualidade" active={activeModule === "qualidade"} onClick={() => setActiveModule("qualidade")}/>
            <SidebarItem icon={Factory} label="Indústrias (PDF)" active={activeModule === "relatorio_industria"} onClick={() => setActiveModule("relatorio_industria")}/>
            ...
            <SidebarItem icon={Factory} label="Indústrias" active={activeModule === "industrias"} onClick={() => setActiveModule("industrias")}/>
            <SidebarItem icon={Store} label="Lojas" active={activeModule === "lojas"} onClick={() => setActiveModule("lojas")}/>
            <SidebarItem icon={Users} label="Promotores" active={activeModule === "promotores"} onClick={() => setActiveModule("promotores")}/>

            <div className="pt-4 pb-2">
              {!collapsed && <p className="px-4 text-[10px] font-bold uppercase tracking-widest text-muted-foreground opacity-50">Operação</p>}
            </div>
            <SidebarItem icon={Settings2} label="Gestão Operacional" active={activeModule === "importacoes"} onClick={() => setActiveModule("importacoes")}/>
            <SidebarItem icon={Upload} label="Importar Checklist" active={activeModule === "checklists"} onClick={() => setActiveModule("checklists")}/>
            <SidebarItem icon={Route} label="Roteiros" active={activeModule === "roteiros"} onClick={() => setActiveModule("roteiros")}/>
            {isAdmin && (<>
                <div className="pt-4 pb-2">
                ...
                <SidebarItem icon={ShieldAlert} label="Limpeza Manual" active={activeModule === "cleanup_admin"} onClick={() => setActiveModule("cleanup_admin")}/>
                <SidebarItem icon={Users} label="Usuários" active={activeModule === "usuarios"} onClick={() => setActiveModule("usuarios")}/>
              </>)}
          </nav>
        </div>

        <div className="p-4 border-t space-y-2">
          <button onClick={() => setCollapsed(!collapsed)} className={cn("w-full flex items-center gap-3 px-4 py-2 text-muted-foreground hover:text-foreground transition-colors", collapsed && "justify-center px-0")}>
            {collapsed ? <ChevronsRight className="h-5 w-5"/> : <ChevronsLeft className="h-5 w-5"/>}
            {!collapsed && "Recolher"}
          </button>
          <button className={cn("w-full flex items-center gap-3 px-4 py-2 text-red-500 hover:text-red-600 hover:bg-red-500/5 transition-colors", collapsed && "justify-center px-0")} onClick={() => signOut()}>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-background/50 backdrop-blur-sm">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 bg-muted/30 p-1.5 rounded-lg border">
              <Select value={String(month)} onValueChange={(v) => setMonth(Number(v))}>
                ...
                <SelectContent>
                  {["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"].map((m, i) => (<SelectItem key={i} value={String(i + 1)}>{m}</SelectItem>))}
                </SelectContent>
              </Select>
              <div className="w-[1px] h-4 bg-muted"/>
              <Input type="number" value={year} onChange={(e) => setYear(Number(e.target.value))} className="w-20 h-8 border-none bg-transparent shadow-none focus-visible:ring-0"/>
            </div>
            {user && (<div className="flex items-center gap-3 pl-4 border-l">
        ...
        <section className="flex-1 overflow-y-auto custom-scrollbar relative">
          <div className="p-8">
            ...
            {activeModule === "importacoes" && (<Mk9ImportModule onSwitchToChecklists={() => setActiveModule("checklists")}/>)}
            {activeModule === "industrias" && <Mk9IndustriesModule />}
            {activeModule === "lojas" && <Mk9StoresModule />}
            {activeModule === "promotores" && <Mk9PromotersModule />}
            {activeModule === "roteiros" && (<Mk9RoutesModule promoters={promotersQ.data ?? []} stores={storesQ.data ?? []} industries={industriesQ.data ?? []}/>)}
            {activeModule === "checklists" && (<Mk9ChecklistImportModule onSwitchToBase={() => setActiveModule("importacoes")}/>)}
            {activeModule === "conciliacao" && <Mk9AuditModule key={auditKey} initialFilters={auditFilters}/>}
            {activeModule === "qualidade" && (<Mk9QualityModule month={month} year={year} onNavigate={(target) => {
                if (target.month)
...
function SidebarItem({ icon: Icon, label, active, onClick }) {
    return (<button onClick={onClick} className={cn("w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200", active
