          ...
          <Mk9Panel className="xl:col-span-1">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Falhas consecutivas</p>
              </div>
            </div>
            ...
            <AnalyticsTable 
              ...
              rows={data.recurrence.slice(0, 6).map(r => [
                <div key={`${r.storeId}-${r.industryName}`} className="flex flex-col">
                  <span className="font-bold text-white uppercase tracking-tighter truncate w-32">{r.storeName}</span>
                  <span className="text-[9px] text-slate-500 font-bold uppercase">{r.uf}</span>
                </div>,
                <span key={`${r.storeId}-${r.industryName}`} className="text-[10px] font-bold text-slate-400 uppercase truncate w-20">{r.industryName}</span>,
                <div key={`${r.storeId}-${r.industryName}`} className="flex gap-1">
                  {r.history.map((h, i) => (
                    <div key={i} className="flex flex-col items-center p-1 rounded bg-white/[0.03] border border-white/5 min-w-[35px]">
                      <span className="text-[6px] font-black text-slate-500">{h.period}</span>
                      <span className={cn("text-[8px] font-black", h.realized === 0 ? "text-rose-500" : "text-white")}>
                        {h.realized}
                      </span>
                    </div>
                  ))}
