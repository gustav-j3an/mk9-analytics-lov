...
import { Users, CheckCircle2, Search, Save, Download, Loader2, Check, X, Plus, Settings } from "lucide-react";
...
import { Mk9PageHeader, Mk9MetricCard, Mk9Panel, Mk9LoadingState, Mk9EmptyState, } from "./mk9/design-system";
...
export function Mk9PresenceModule() {
    ...
    const saveMutation = useMutation({
        mutationFn: (items) => saveFn({ data: { date, items } }),
        onSuccess: () => {
    ...
    const handleStatusChange = (promoterId, status) => {
        setLocalPresence(prev => ({
            ...prev,
            [promoterId]: { ...prev[promoterId], status }
        }));
    };
    const handleObservationChange = (promoterId, observation) => {
        setLocalPresence(prev => ({
            ...prev,
    ...
    const localStats = useMemo(() => {
        ...
        Object.values(localPresence).forEach(p => {
            if (p.status === 'PRESENT')
                ...
                s.absent++;
            else if (p.status === 'MEDICAL_CERTIFICATE')
                s.medical++;
    ...
    return (<div className="space-y-8 animate-fade-up">
      <Mk9PageHeader title="Controle de Presença" subtitle="Registro operacional diário de comparecimento" icon={CheckCircle2} actions={<div className="flex items-center gap-2 glass-command p-1.5 rounded-xl border border-border/50">
            <div className="relative group/date shrink-0">
              <Input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="h-8 w-[170px] bg-input/50 border-border/50 text-[10px] font-bold text-foreground uppercase tracking-wider pr-8"/>
            </div>
            <Sheet>
            ...
            <Button onClick={handleSave} disabled={saveMutation.isPending} className="h-8 gap-2 bg-primary hover:bg-primary/90 text-primary-foreground border-none uppercase text-[10px] font-black tracking-widest px-4">
              {saveMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin"/> : <Save className="h-4 w-4"/>}
              Salvar
            </Button>
            <Button onClick={exportToExcel} variant="outline" className="h-8 gap-2 border-border text-muted-foreground hover:text-foreground uppercase text-[10px] font-black tracking-widest px-4">
      ...
      <Mk9Panel>
        <div className="space-y-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <Button variant="outline" size="sm" onClick={markAllPresent} className="h-9 sm:h-7 w-full sm:w-auto border-emerald-500/20 text-emerald-500 hover:bg-emerald-500/10 text-[9px] font-black uppercase tracking-widest">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="relative w-full">
              ...
              <Input placeholder="Nome ou matrícula..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 h-9 w-full bg-popover border-border text-foreground text-xs"/>
            </div>
        ...
        <div className="rounded-xl border border-border/50 overflow-hidden">
          {listLoading ? (<Mk9LoadingState message="Carregando promotores..."/>) : !presenceItems || presenceItems.length === 0 ? (<Mk9EmptyState message="Nenhum promotor encontrado. Verifique se há promotores ativos no cadastro ou ajuste os filtros."/>) : (<div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                return (<tr key={item.id} className="border-b border-border/50 last:border-0 hover:bg-muted/10 transition-colors group">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center gap-1">
                            <PresenceButton active={local.status === 'PRESENT'} onClick={() => handleStatusChange(item.id, 'PRESENT')} variant="present"/>
                            <PresenceButton active={local.status === 'ABSENT'} onClick={() => handleStatusChange(item.id, 'ABSENT')} variant="absent"/>
                            <PresenceButton active={local.status === 'MEDICAL_CERTIFICATE'} onClick={() => handleStatusChange(item.id, 'MEDICAL_CERTIFICATE')} variant="medical"/>
                            <PresenceButton active={local.status === 'VACATION'} onClick={() => handleStatusChange(item.id, 'VACATION')} variant="vacation"/>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <Input placeholder="..." value={local.observation} onChange={(e) => handleObservationChange(item.id, e.target.value)} className="h-7 text-[10px] bg-transparent border-border/50 focus:border-primary text-foreground/80 w-full"/>
                        </td>
                      </tr>);
            })}
                </tbody>
...
function PresenceButton({ active, onClick, variant }) {
    const configs = {
    ...
    return (<button onClick={onClick} className={cn("flex items-center gap-1.5 px-3 h-7 rounded-md text-[9px] font-black uppercase tracking-tight transition-all border", active ? config.activeClass : config.inactiveClass)}>
