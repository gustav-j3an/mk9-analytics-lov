...
import {
  reconcileRun, reconcileSummary, reconcileIgnore, reconcileUndoReview,
  reconcileListPaged, reconcileDetail, reconcileFindCandidates,
  reconcileManualMatch, reconcileAcceptDivergence, reconcileSearchStores,
...
type TabKey = "all" | "matched" | "pending" | "unplanned" | "manual" | "not_completed" | "store_not_found";
...
export function Mk9ReconciliationModule() {
  ...
  useEffect(() => { setPage(1); }, [tab, industryId, promoterId, uf, importId, search, month, year]);
  ...
  const importsQ = useQuery({ queryKey: ["mk9-reco", "imports", scope], queryFn: () => importsFn({ data: scope }) });
  const summaryQ = useQuery({ queryKey: ["mk9-reco", "summary", scope], queryFn: () => summaryFn({ data: scope }) });
  ...
  const runMut = useMutation({
    ...
    onSuccess: () => { toast.success("Conciliação executada"); qc.invalidateQueries({ queryKey: ["mk9-reco"] }); },
  ...
  const ignoreMut = useMutation({
    ...
    onSuccess: () => { toast.success("Marcado como ignorado"); qc.invalidateQueries({ queryKey: ["mk9-reco"] }); setIgnoreTarget(null); },
    onError: (e: any) => toast.error(`Falha: ${e?.message ?? e}`),
  });
  const undoMut = useMutation({
    mutationFn: (id: string) => undoFn({ data: { reconciliationId: id } }),
    onSuccess: () => { toast.success("Revisão desfeita"); qc.invalidateQueries({ queryKey: ["mk9-reco"] }); },
    onError: (e: any) => toast.error(`Falha: ${e?.message ?? e}`),
  });
  ...
  function exportCsv() {
    const cols = ["status","industria","promotor","loja","uf","data_planejada","data_realizada","diff_dias","score","tipo","revisao_manual","observacao","arquivo"];
    ...
    <div className="space-y-6">
              ...
              <Table>
                ...
                <TableBody>
                  {rows.map((r: any) => {
                    ...
                    return (
                      <>
                        <TableRow key={r.id} className={r.reviewed_manually ? "bg-primary/5" : undefined}>
                          <TableCell>
                            <button onClick={() => setExpanded(isOpen ? null : r.id)} className="text-muted-foreground hover:text-foreground">
                          ...
                          <TableCell>{r.promoter?.name ?? (st === "UNPLANNED_VISIT" ? "Não identificado" : "—")}</TableCell>
      ...
      {reviewTarget && (
        <ReviewDialog
          ...
          onDone={() => { setReviewTarget(null); qc.invalidateQueries({ queryKey: ["mk9-reco"] }); }}
        />
      )}
      {storeTarget && (
        <StoreLinkDialog
          ...
          onDone={() => { setStoreTarget(null); qc.invalidateQueries({ queryKey: ["mk9-reco"] }); }}
        />
      )}
      {ignoreTarget && (
        <IgnoreDialog
...
function RowActions({
  row, onReview, onLinkStore, onIgnore, onUndo,
}: {
  row: any;
  onReview: () => void;
  onLinkStore: () => void;
  ...
  const canReview = st === "AMBIGUOUS" || st === "DATE_DIVERGENCE" || st === "UNPLANNED_VISIT" || st === "DUPLICATE_ACTUAL";
...
function RowDetail({ id }: { id: string }) {
  ...
  const q = useQuery({ queryKey: ["mk9-reco", "detail", id], queryFn: () => detailFn({ data: { id } }) });
  if (q.isLoading) return <div className="p-4 text-sm text-muted-foreground flex items-center gap-2"><Loader2 className="h-4 w-4 animate-spin" /> Carregando detalhe…</div>;
  if (q.error || !q.data) return <div className="p-4 text-sm text-rose-300">Erro ao carregar detalhe.</div>;
  const { reconciliation: r, planned, actual, importInfo } = q.data;
  const weekday = planned?.scheduled_date ? WEEKDAY_PT[new Date(planned.scheduled_date + "T00:00:00Z").getUTCDay()] : null;
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3 p-4">
      ...
      <DetailBlock title="Conciliação">
        ...
        <DetailRow k="Data revisão" v={r.reviewed_at ? new Date(r.reviewed_at).toLocaleString("pt-BR") : "—"} />
...
function ReviewDialog({ row, onClose, onDone }: { row: any; onClose: () => void; onDone: () => void }) {
  const st = row.status as ReconciliationStatus;
  ...
  const linkMut = useMutation({
    mutationFn: () => manualFn({ data: { actualVisitId: row.actual_visit_id, plannedVisitId: selected!, notes: notes || null } }),
    onSuccess: () => { toast.success("Vínculo manual salvo"); onDone(); },
    onError: (e: any) => toast.error(`Falha: ${e?.message ?? e}`),
  });
  const acceptMut = useMutation({
    mutationFn: () => acceptFn({ data: { reconciliationId: row.id, notes: notes || null } }),
    onSuccess: () => { toast.success("Divergência aceita"); onDone(); },
    onError: (e: any) => toast.error(`Falha: ${e?.message ?? e}`),
  });
    ...
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Realizada em {fmtDate(row.actual_date)} · {row.store?.name ?? row.raw_store_name ?? "—"} · {row.industry?.name ?? "—"}
          </DialogDescription>
        </DialogHeader>

        {st === "DATE_DIVERGENCE" && (
          <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-3 text-sm space-y-1">
            <div>Planejado: <strong>{fmtDate(row.planned_date)}</strong></div>
            <div>Realizado: <strong>{fmtDate(row.actual_date)}</strong></div>
            <div>Diferença: <strong>{row.date_diff_days} dia(s)</strong></div>
          </div>
        )}
        ...
        {isUnplanned || st === "DATE_DIVERGENCE" ? (
          <div className="space-y-2 max-h-[360px] overflow-auto">
            <div className="text-xs text-muted-foreground">
              {candidatesQ.isLoading ? "Buscando candidatos…" : "Selecione uma visita planejada para vincular:"}
            </div>
            {(candidatesQ.data?.candidates ?? []).length === 0 && !candidatesQ.isLoading && (
              <div className="text-sm text-muted-foreground">Nenhum candidato encontrado em ±14 dias.</div>
            )}
              ...
              >
                ...
                <div className="flex-1 text-sm">
                  <div className="font-medium">
                    {c.store?.name ?? "—"} <span className="text-muted-foreground">· {c.store?.uf ?? "—"}</span>
                    {c.sameStore && <Badge variant="outline" className="ml-2 text-[10px]">mesma loja</Badge>}
                  </div>
        ...
        <DialogFooter className="gap-2">
          ...
          {st === "DATE_DIVERGENCE" && (
            <Button variant="secondary" onClick={() => acceptMut.mutate()} disabled={acceptMut.isPending}>
              {acceptMut.isPending && <Loader2 className="h-4 w-4 mr-2 animate-spin" />} Aceitar divergência
            </Button>
          )}
          <Button
          ...
          >
            {linkMut.isPending && <Loader2 className="h-4 w-4 mr-2 animate-spin" />} Vincular manualmente
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function StoreLinkDialog({ row, onClose, onDone }: { row: any; onClose: () => void; onDone: () => void }) {
  ...
  const linkMut = useMutation({
    mutationFn: () => linkFn({ data: { reconciliationId: row.id, storeId: selected!, notes: notes || null } }),
    onSuccess: () => { toast.success("Loja vinculada"); onDone(); },
    onError: (e: any) => toast.error(`Falha: ${e?.message ?? e}`),
  });
    ...
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Loja original: <strong>{row.raw_store_name ?? "—"}</strong> · UF: {row.raw_store_uf ?? "—"} · Data: {fmtDate(row.actual_date)} · Indústria: {row.industry?.name ?? "—"}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar loja pelo nome…" className="pl-9" />
          </div>
          <Select value={uf} onValueChange={setUf}>
        ...
        <div className="max-h-[320px] overflow-auto space-y-1.5">
          ...
          {(storesQ.data ?? []).map((st: any) => (
            ...
            >
              <input type="radio" name="store" checked={selected === st.id} onChange={() => setSelected(st.id)} />
              <div className="flex-1">
                <div className="font-medium">{st.name}</div>
                <div className="text-xs text-muted-foreground">{st.chain ?? "—"} · {st.uf ?? "—"}</div>
              </div>
            </label>
          ))}
        </div>
...
// ---------- Modal de ignorar ----------
function IgnoreDialog({ row, pending, onClose, onConfirm }: {
  row: any; pending: boolean; onClose: () => void; onConfirm: (notes: string) => void;
}) {
  const [notes, setNotes] = useState("");
  return (
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {STATUS_LABELS_PT[row.status as ReconciliationStatus]} · {row.store?.name ?? row.raw_store_name ?? "—"} · {fmtDate(row.planned_date ?? row.actual_date)}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-1">
          <label className="text-xs text-muted-foreground">Motivo / observação</label>
          <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} placeholder="Ex.: loja fechada, promotor afastado, roteiro alterado…" />
        </div>
        <DialogFooter>
          <Button variant="ghost" onClick={onClose}>Cancelar</Button>
