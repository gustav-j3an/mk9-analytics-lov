...
function drawHeader(ctx: PdfCtx) {
  ...
  ctx.page.drawText("Lojas não atendidas", {
    x: PAGE.w - MARGIN - ctx.font.widthOfTextAtSize("Lojas não atendidas", 10),
    y: PAGE.h - 30,
    size: 10,
    font: ctx.font,
    color: COLOR_MUTED,
...
export async function renderUnattendedPdf(
  report: IndustryReport,
  year: number,
  month: number,
): Promise<Uint8Array> {
  ...
  // Resumo Executivo
  ctx.page.drawText("RESUMO EXECUTIVO", {
    x: MARGIN,
    y: ctx.y,
    size: 12,
    font: ctx.fontB,
  ...
  const summary = [
    ...
    [
      "Visitas contratadas nessas lojas:",
      String(unattendedStores.reduce((sum, s) => sum + s.expected, 0)),
    ],
  ];
  ...
  // UFs afetadas
  ctx.y -= 10;
  ctx.page.drawText("UFs afetadas:", {
    x: MARGIN,
    y: ctx.y,
    size: 9,
    font: ctx.fontB,
  ...
  ufList.forEach(([uf, count]) => {
    ctx.page.drawText(`${uf} — ${count} lojas`, {
      x: MARGIN + 10,
      y: ctx.y,
      size: 8,
      font: ctx.font,
  ...
  if (unattendedStores.length === 0) {
    ctx.page.drawText(
      "Todas as lojas contratadas desta indústria tiveram pelo menos um atendimento no período selecionado.",
      {
        x: MARGIN,
        y: ctx.y,
        size: 10,
    ...
    for (const uf of sortedUfs) {
      ...
      for (const [promoter, stores] of promoters) {
        ensureSpace(ctx, 30);
        ctx.page.drawText(`Promotor: ${promoter}`, {
          x: MARGIN + 10,
          y: ctx.y,
          size: 9,
          font: ctx.fontB,
        ...
        ctx.page.drawText(subStr, {
          x: MARGIN + 20 + ctx.fontB.widthOfTextAtSize(`Promotor: ${promoter}`, 9),
          y: ctx.y,
          size: 8,
          font: ctx.font,
          color: COLOR_MUTED,
