...
function drawFooter(ctx, pageNum, total) {
    const y = 30;
...
export async function renderUnattendedPdf(report, year, month) {
    ...
    const ctx = { pdf, page, y: PAGE.h - 60, font, fontB, report, unattendedStores };
    drawHeader(ctx);
    // Resumo Executivo
    ...
    if (unattendedStores.length === 0) {
        ...
        unattendedStores.forEach(s => {
            ...
            if (!groups.has(uf))
                groups.set(uf, new Map());
            const ufGroup = groups.get(uf);
            if (!ufGroup.has(promoter))
                ufGroup.set(promoter, []);
        ...
        for (const uf of sortedUfs) {
            ...
            for (const [promoter, stores] of promoters) {
                ...
                stores.sort((a, b) => a.storeName.localeCompare(b.storeName)).forEach((s, idx) => {
                    ...
                    if (idx % 2 === 1)
                        ctx.page.drawRectangle({ x: MARGIN, y: ctx.y - rowH, width: CONTENT_W, height: rowH, color: COLOR_ROW_ALT });
                    let cellX = MARGIN + 5;
...
export function unattendedPdfFileName(report, year, month) {
