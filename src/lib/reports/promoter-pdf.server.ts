...
function drawHeader(ctx: PdfCtx) {
  ...
  ctx.page.drawText("ROTEIRO DO PROMOTOR", { x: PAGE.w - MARGIN - ctx.font.widthOfTextAtSize("ROTEIRO DO PROMOTOR", 10), y: PAGE.h - 30, size: 10, font: ctx.font, color: COLOR_MUTED });
...
export async function renderPromoterRoutePdf(input: {
  ...
  ctx.page.drawText("RESUMO DO ROTEIRO", { x: MARGIN, y: ctx.y, size: 12, font: fontB, color: COLOR_BRAND });
  ctx.y -= 25;
  ...
  for (const [label, val] of info) {
    if (!label) { ctx.y -= 10; continue; }
    ctx.page.drawText(label, { x: MARGIN, y: ctx.y, size: 9, font: fontB, color: COLOR_TEXT });
    ctx.page.drawText(sanitizePdfText(val), { x: MARGIN + 120, y: ctx.y, size: 9, font: font, color: COLOR_TEXT });
    ctx.y -= 14;
  }

  ctx.y -= 20;
  ctx.page.drawLine({ start: { x: MARGIN, y: ctx.y + 10 }, end: { x: PAGE.w - MARGIN, y: ctx.y + 10 }, thickness: 0.5, color: COLOR_LINE });
  ...
  // Renderiza cada dia
  for (let wd = 1; wd <= 6; wd++) { // Segunda a Sábado (padrão)
    ...
    ctx.page.drawText(dayLabel, { x: MARGIN + 5, y: ctx.y - 11, size: 9, font: fontB, color: COLOR_BRAND });
    ctx.y -= 25;

    if (stores.length === 0) {
      ctx.page.drawText("Sem visitas programadas.", { x: MARGIN + 10, y: ctx.y, size: 8, font: font, color: COLOR_MUTED });
      ctx.y -= 15;
    } else {
      // Ordena por Loja
      ...
      for (const s of stores) {
        ensureSpace(ctx, 15);
        const storeLine = sanitizePdfText(`${s.chain ? `${s.chain} · ` : ""}${s.storeName} (${s.uf})`);
        ctx.page.drawText("•", { x: MARGIN + 5, y: ctx.y, size: 8, font: fontB, color: COLOR_BRAND });
        ctx.page.drawText(storeLine, { x: MARGIN + 15, y: ctx.y, size: 8, font: font, color: COLOR_TEXT });
        const indW = font.widthOfTextAtSize(sanitizePdfText(s.industryName), 7);
        ctx.page.drawText(sanitizePdfText(s.industryName), { x: PAGE.w - MARGIN - indW, y: ctx.y, size: 7, font: font, color: COLOR_MUTED });
        ctx.y -= 12;
      }
      ctx.y -= 5;
    }
  ...
  ensureSpace(ctx, 120);
  ctx.y -= 20;
  ctx.page.drawRectangle({ x: MARGIN, y: ctx.y - 100, width: CONTENT_W, height: 100, color: rgb(0.98, 0.98, 0.98), borderColor: COLOR_LINE, borderWidth: 0.5 });
  
  let subY = ctx.y - 20;
  ctx.page.drawText("RESUMO POR DIA", { x: MARGIN + 20, y: subY, size: 9, font: fontB, color: COLOR_TEXT });
  subY -= 20;
  ...
  daysToShow.forEach((wd, i) => {
    ...
    ctx.page.drawText(`${label}: ${Math.round(weekdayTotalVisits[wd])}`, { x, y: y, size: 8, font: font, color: COLOR_TEXT });
  });

  const totalStr = `TOTAL: ${totalVisits} visitas`;
  const totalW = fontB.widthOfTextAtSize(totalStr, 10);
  ctx.page.drawText(totalStr, { x: PAGE.w - MARGIN - 20 - totalW, y: subY - 15, size: 10, font: fontB, color: COLOR_BRAND });
...
export function promoterPdfFileName(name: string, year: number, month: number): string {
  const clean = name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase().replace(/[^A-Z0-9]+/g, "_");
