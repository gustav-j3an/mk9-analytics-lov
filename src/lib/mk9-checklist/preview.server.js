...
import { cancelPreviousPreviews, createChecklistImport, loadIndustry, loadStoresIndex, savePreviewSnapshot, updateImportStatus, writeValidationReport, } from "./persistence.server";
import { createHash } from "crypto";
const SIMILARITY_THRESHOLD = 0.95;
export async function runChecklistPreview(input, diagnostics) {
    ...
    if (parsed.marks.length === 0 && parsed.stores.length === 0) {
        const payload = buildRichError(new Error("Planilha vazia ou fora do modelo esperado. Não foi possível localizar cabeçalho com coluna 'Loja' + colunas de datas."), {
            step: "parse-workbook",
    ...
    if (parsed.firstDate) {
        ...
        if (datesInside === 0 && totalVisits > 0) {
            ...
            const conflictPayload = buildRichError(new Error(`As datas do arquivo estão totalmente fora do período operacional esperado para ${selectedCompetence} (${window.startDate} a ${window.endDate}).`), {
        ...
        if (insideRatio < 0.8) {
            const selectedCompetence = `${input.operationMonth.toString().padStart(2, "0")}/${input.operationYear}`;
            const warningPayload = buildRichError(new Error(`As datas da planilha estão majoritariamente fora do período esperado para ${selectedCompetence}.`), {
                step: "validate-competence",
                function: "checklistPreview",
                extra: {
            ...
            diagnostics.error("competence-warning", "Baixa correlação com o período selecionado", warningPayload.extra);
        }
    }
    ...
    function resolve(normalized, uf) {
        ...
        if (exact) {
            const r = { kind: "found", storeId: exact.id, matchedName: exact.name };
        ...
        if (unique) {
            const r = { kind: "found", storeId: unique.id, matchedName: unique.name };
            resolveCache.set(key, r);
            return r;
        }
        ...
        if (fuzzyExact) {
            const r = { kind: "found", storeId: fuzzyExact.id, matchedName: fuzzyExact.name };
            resolveCache.set(key, r);
            return r;
        }
    ...
    const storesLinked = new Set();
    const storesNew = new Set();
    const storeFrequencyByKey = new Map();
    for (const mark of parsed.marks) {
        ...
        if (r.kind === "linked_by_similarity")
            storesLinked.add(key);
        else
            storesFound.add(key);
        if (!dateStr) {
            items.push({
    ...
    for (const s of parsed.stores) {
        ...
        if (!storesSeen.has(key))
            storesSeen.add(key);
        if (r.kind === "found")
            storesFound.add(key);
        else if (r.kind === "linked_by_similarity")
            storesLinked.add(key);
        else
            storesNew.add(key);
    ...
    const biweeklyFrequencies = storeFrequencies.filter((s) => evaluateFrequencyConsistency(s.weeklyFrequency, s.monthlyFrequency).isBiweekly).length;
    ...
    if (previousData) {
        preview.previousImport = {
            ...
            counters: previousData.counters,
        };
    }
    ...
    if (preview.validation)
        await writeValidationReport(importId, preview.validation);
