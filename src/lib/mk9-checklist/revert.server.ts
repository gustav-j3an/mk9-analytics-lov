...
export async function getRevertPreview(importId: string): Promise<RevertPreview> {
  ...
  // If an import created a version, and then a MANUAL version was created for the same store/industry with valid_from >= this version's valid_from.
  ...
  // This is a simplified check for manual dependencies.
  // In a real scenario, we'd check if any of the stores in this import have a newer manual version.
  const manualDependenciesCount = 0; // Simplified for now
...
export async function executeRevert(importId: string, options: { reason: string; actorId?: string }) {
  const { data, error } = await supabaseAdmin.rpc("mk9_revert_checklist_import", {
    _import_id: importId,
...
export async function executeCorrection(importId: string, options: { targetMonth: number; targetYear: number; reason: string; actorId?: string }) {
  ...
  const { createChecklistImport, savePreviewSnapshot } = await import("./persistence.server");
  ...
  const commitRes = await checklistCommit({
    importId: newImp.id,
    industryId: imp.industry_id,
    operationMonth: options.targetMonth,
    operationYear: options.targetYear,
