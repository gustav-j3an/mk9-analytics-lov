...
function firstStackFrame(stack?: string): { file?: string; line?: number } {
  if (!stack) return {};
...
function fromZod(err: ZodError): RichErrorPayload["validation"] {
...
function fromPostgrest(err: any): RichErrorPayload["database"] | undefined {
...
export function buildRichError(
  ...
  if (ctx.parser) base.parser = ctx.parser;
  if (ctx.extra) base.extra = ctx.extra;
  ...
  if (err instanceof Error) {
    ...
    const db = fromPostgrest(err as any) ?? fromPostgrest((err as any).cause);
