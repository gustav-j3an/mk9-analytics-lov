...
function headerMatch(cell, keywords) {
...
function detectDateColumn(cell) {
...
export function parseChecklistWorkbook(buffer, filename, options = {}) {
    const debug = (step, message, data) => {
    ...
    for (const sheetName of wb.SheetNames) {
        ...
        for (let r = 0; r < Math.min(rows.length, 40); r++) {
            ...
            if (localStoreCol >= 0 && filtered.length >= 3) {
                ...
                for (let c = startCol; c < endCol; c++) {
                    ...
                    out.warnings.push(`Cabeçalho de data corrigido automaticamente na coluna ${c + 1} (linha ${r + 1}): "${rawStr}" interpretado como ${iso.split("-").reverse().join("/")}.`);
                }
        ...
        for (let r = headerRow + 1; r < rows.length; r++) {
            ...
            if (/^(total|totais|geral|subtotal)/i.test(storeName))
                continue;
            const uf = ufValue;
            ...
            if (realizado !== null && Number.isFinite(realizado))
                sheetRealizadoSum += realizado;
            if (monthly !== null && Number.isFinite(monthly))
                out.monthlyFrequencySum += monthly;
    ...
    if (out.realizadoSum > 0 && out.marks.length !== out.realizadoSum) {
        out.warnings.push(`Alerta de divergência (REALIZED_SUMMARY_MISMATCH): a coluna REALIZADO informa ${out.realizadoSum} visitas, mas foram identificadas ${out.marks.length} marcações reais nas datas. O sistema utilizará as ${out.marks.length} marcações confirmadas.`);
    }
    else if (out.realizadoSum === 0 && out.marks.length > 0) {
        // Caso a coluna REALIZADO esteja vazia mas existam marcações
        out.warnings.push(`Coluna REALIZADO zerada ou ausente, mas foram identificadas ${out.marks.length} marcações reais nas datas.`);
    }
