...
import { NEAR_DATE_SCORE, NEAR_DATE_WINDOW_DAYS, } from "./types";
/** Aplica o escopo de acesso a uma consulta de conciliação (indústria/loja/promotor). */
export function applyReconAccess(q, access) {
    if (!access)
        return q;
    if (access.allowedIndustryIds)
        q = q.in("industry_id", access.allowedIndustryIds);
    if (access.allowedStoreIds)
        q = q.in("store_id", access.allowedStoreIds);
    if (access.allowedPromoterIds)
        q = q.in("promoter_id", access.allowedPromoterIds);
    return q;
}
/** Filtro de UF pós-consulta (UF vive na loja relacionada). */
export function filterRowsByAccessUf(rows, access) {
...
export function accessIsEmpty(access) {
    return (access?.allowedIndustryIds?.length === 0 ||
...
async function loadScope(scope) {
    const plannedQ = supabaseAdmin
        ...
        .lt("scheduled_date", `${scope.operationMonth === 12 ? scope.operationYear + 1 : scope.operationYear}-${String(scope.operationMonth === 12 ? 1 : scope.operationMonth + 1).padStart(2, "0")}-01`);
    ...
    const existingQ = supabaseAdmin
        ...
        .eq("operation_month", scope.operationMonth);
    if (scope.industryId)
        existingQ.eq("industry_id", scope.industryId);
...
async function loadStoreNotFoundFromImports(scope) {
    const q = supabaseAdmin
        ...
        .eq("status", "done");
    if (scope.industryId)
        q.eq("industry_id", scope.industryId);
    if (scope.sourceImportId)
        q.eq("id", scope.sourceImportId);
...
export async function reconcile(scope) {
    const { planned, actual, existing } = await loadScope(scope);
    // Preserve manual/ignored reconciliations
    const preservedByActual = new Map();
    const preservedByPlanned = new Map();
    const preserveIds = new Set();
    for (const r of existing) {
        const isManual = r.reviewed_manually || r.status === "MANUALLY_MATCHED" || r.status === "IGNORED";
        if (isManual) {
            preserveIds.add(r.id);
            if (r.actual_visit_id)
                preservedByActual.set(r.actual_visit_id, r);
            if (r.planned_visit_id)
                preservedByPlanned.set(r.planned_visit_id, r);
        }
    }
    ...
    // Build indices
    const key = (industry, store, date) => `${industry}|${store}|${date}`;
    const plannedByKey = new Map();
    const plannedByStore = new Map(); // industry|store -> planned rows
    for (const p of planned) {
        if (preservedByPlanned.has(p.id))
            continue; // manual owns it
    ...
    for (const a of actual) {
        if (consumedActual.has(a.id) || preservedByActual.has(a.id))
            continue;
        const candidates = (plannedByStore.get(`${a.industry_id}|${a.store_id}`) ?? []).filter((p) => !consumedPlanned.has(p.id));
    ...
    for (const p of planned) {
        if (consumedPlanned.has(p.id) || preservedByPlanned.has(p.id))
            continue;
        inserts.push(baseRow({
            planned_visit_id: p.id,
...
export async function summarize(scope) {
    let q = supabaseAdmin
        .from("mk9_visit_reconciliations")
        ...
        .eq("operation_month", scope.operationMonth);
    if (scope.industryId)
        q = q.eq("industry_id", scope.industryId);
...
export async function undoReview(reconciliationId) {
    const { error } = await supabaseAdmin
        .from("mk9_visit_reconciliations")
...
export async function listReconciliations(scope, limit = 500) {
    const q = supabaseAdmin
        .from("mk9_visit_reconciliations")
        .select("id, status, match_score, match_type, planned_date, actual_date, date_diff_days, reviewed_manually, raw_store_name, raw_store_uf, notes, industry:mk9_industries(id,name), store:mk9_stores(id,name,uf), promoter:mk9_promoters(id,name), planned_visit_id, actual_visit_id, source_import_id")
        .eq("operation_year", scope.operationYear)
        .eq("operation_month", scope.operationMonth)
    ...
    let qq = q;
    if (scope.industryId)
        qq = qq.eq("industry_id", scope.industryId);
    if (scope.sourceImportId)
        qq = qq.eq("source_import_id", scope.sourceImportId);
...
export async function listReconciliationsPaged(f) {
    ...
    let q = supabaseAdmin
        .from("mk9_visit_reconciliations")
        .select("id, status, match_score, match_type, planned_date, actual_date, date_diff_days, reviewed_manually, reviewed_by, reviewed_at, raw_store_name, raw_store_uf, notes, industry:mk9_industries(id,name), store:mk9_stores(id,name,uf), promoter:mk9_promoters(id,name), planned_visit_id, actual_visit_id, source_import_id, candidates", { count: "exact" })
        .eq("operation_year", f.operationYear)
        .eq("operation_month", f.operationMonth);
    if (f.industryId)
        q = q.eq("industry_id", f.industryId);
    if (f.sourceImportId)
        q = q.eq("source_import_id", f.sourceImportId);
    if (f.promoterId)
        q = q.eq("promoter_id", f.promoterId);
    if (f.storeId)
        q = q.eq("store_id", f.storeId);
    ...
    if (error)
        throw new Error(error.message);
    let rows = filterRowsByAccessUf((data ?? []), f.access);
    if (f.uf) {
...
// ============ DETALHE EXPANDIDO ============
export async function getReconciliationDetail(id, access) {
    ...
    if (access) {
        ...
        const outOfScope = (access.allowedIndustryIds && !access.allowedIndustryIds.includes(r.industry_id)) ||
            (access.allowedStoreIds &&
                (!r.store_id || !access.allowedStoreIds.includes(r.store_id))) ||
            (access.allowedPromoterIds &&
                (!r.promoter_id || !access.allowedPromoterIds.includes(r.promoter_id))) ||
            (access.allowedUfs &&
                !access.allowedUfs.includes((r.store?.uf ?? r.raw_store_uf)));
        if (outOfScope)
...
export async function findPlannedCandidates(input) {
    ...
    enriched.sort((x, y) => {
        if (x.sameStore !== y.sameStore)
            return x.sameStore ? -1 : 1;
...
export async function searchStores(input) {
    ...
    if (access?.allowedUfs?.length === 0 || access?.allowedStoreIds?.length === 0)
        return [];
    ...
    if (input.uf)
        q = q.eq("uf", input.uf.toUpperCase());
    else if (access?.allowedUfs)
        q = q.in("uf", access.allowedUfs);
    if (access?.allowedStoreIds)
        q = q.in("id", access.allowedStoreIds);
    const { data, error } = await q;
    if (error)
        throw new Error(error.message);
    return data ?? [];
...
export async function linkStoreToReconciliation(input) {
    ...
    if (error)
        throw new Error(error.message);
    if (!r.actual_date)
        throw new Error("Reconciliação sem data realizada");
    ...
    if (candidates && candidates.length > 0) {
        const best = [...candidates].sort((a, b) => Math.abs(diffDays(a.scheduled_date, r.actual_date)) -
            Math.abs(diffDays(b.scheduled_date, r.actual_date)))[0];
        await manualMatch({
            actualVisitId: av.id,
...
export async function listChecklistImportsInScope(scope) {
    ...
    if (scope.industryId)
        ...
        q = q.in("industry_id", allowedIndustries);
    const { data, error } = await q;
    if (error)
        throw new Error(error.message);
    return data ?? [];
