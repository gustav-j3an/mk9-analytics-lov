...
export const getPresenceList = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    let promotersQuery = supabaseAdmin
      .from('mk9_promoters')
      .select('id, name, registration_number, uf, supervisor_id')
      .eq('active', true);

    if (data.filters?.search) {
      promotersQuery = promotersQuery.or(`name.ilike.%${data.filters.search}%,registration_number.ilike.%${data.filters.search}%`);
    }
    ...
    return promoters.map(p => {
      const pData = presence.find(pr => pr.promoter_id === p.id);
      return {
        ...p,
