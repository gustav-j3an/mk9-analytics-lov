...
import { normalizeMk9Roles } from "./roles";
/**
 ...
 */
function mk9AuthError(statusCode, message) {
...
const Mk9AuthorizationError = (message) => mk9AuthError(403, message);
const Mk9UnauthenticatedError = (message) => mk9AuthError(401, message);
/**
 ...
 */
function authFailure(fromHttpRoute, statusCode, message) {
    if (fromHttpRoute)
        return mk9AuthError(statusCode, message);
...
export function devBypassAllowed(request) {
    if (process.env.MK9_DISABLE_DEV_BYPASS === "1")
        return false;
...
export function sanitizeServerError(error) {
    ...
    const looksInternal = /(select|insert|update|delete|from\s+mk9_|relation|constraint|policy|row-level|permission denied|pg[a-z_]*|\/[a-z0-9_.\-\/]+\.(ts|tsx|js)|service_role|supabase)/i.test(raw);
    return looksInternal || raw.length > 200 ? "Não foi possível concluir a operação." : raw;
}
function opaqueFetch(key) {
    return (input, init) => {
 ...
 */
export async function requireMk9Role(required, opts) {
    const fromHttpRoute = Boolean(opts?.request);
    const request = opts?.request ?? getRequest();
    const fail = (code, msg) => authFailure(fromHttpRoute, code, msg);
    const authHeader = request?.headers.get("authorization") ?? null;
    if (!authHeader) {
        if (!devBypassAllowed(request)) {
            throw fail(401, "Autenticação obrigatória. Faça login para continuar.");
        }
        console.warn(`[MK9-AUTH] dev-bypass local: ação exige ${required.join("|")} — sem Authorization header. ` +
    ...
    if (!ok) {
        throw fail(403, `Usuário sem permissão para executar esta ação. Papel exigido: ${required.join(" ou ")}.`);
    }
 ...
 */
export async function logAudit(ctx, action, entity, entityId, metadata = {}) {
    try {
