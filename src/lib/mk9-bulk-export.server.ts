...
export async function processBulkExportItem(
  ...
  try {
    ...
    console.log(
      `[UNVISITED REPORT SUCCESS] industryId=${industryId} duration=${Date.now() - startTime}ms`,
    );
...
export async function generateBulkConsolidatedPdf(
  ...
  page.drawText("Relatório de Lojas Não Atendidas - Consolidado", {
    x: 50,
    y: 710,
    size: 18,
    font: fontBold,
  ...
  page.drawText(`Competência: ${monthLabel} / ${exportRecord.competence_year}`, {
    x: 50,
    y: 680,
    size: 12,
    font: fontRegular,
  ...
  const validResults = results.filter(
    (r) => r.report && r.report.stores.some((s) => s.expected > 0 && s.actual === 0),
  );
  page.drawText(`Indústrias analisadas: ${results.length}`, {
    x: 50,
    y: 630,
    size: 12,
    font: fontRegular,
  });
  page.drawText(`Indústrias com pendência: ${validResults.length}`, {
    x: 50,
    y: 610,
    size: 12,
    font: fontRegular,
  ...
  const totalStores = validResults.reduce(
    (sum, r) =>
      sum + (r.report?.stores.filter((s) => s.expected > 0 && s.actual === 0).length || 0),
    0,
  );
  page.drawText(`Total de lojas com zero visitas: ${totalStores}`, {
    x: 50,
    y: 590,
    size: 12,
    font: fontRegular,
