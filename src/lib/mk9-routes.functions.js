...
export const mk9RoutesListVersioned = createServerFn({ method: "POST" })
    ...
    const { industryFilter, ufFilter, storeFilter, promoterFilter } = await import("@/lib/mk9-auth/access-scope.server");
    ...
    if (ind.outOfScope || store.outOfScope || promoter.outOfScope || ufs.outOfScope)
        return [];
    if (ind.ids?.length === 0 || store.ids?.length === 0 || ufs.ids?.length === 0)
        return [];
    let q = supabaseAdmin
        .from("mk9_planned_routes")
        .select("id, weekday, valid_from, valid_until, is_active, archived_at, source_sheet, promoter:mk9_promoters(id,name), store:mk9_stores(id,name,chain,uf), industry:mk9_industries(id,name)")
        .is("archived_at", null)
        .lte("valid_from", ref)
    ...
    if (promoter.ids)
        q = q.in("promoter_id", promoter.ids);
    if (ind.ids)
        q = q.in("industry_id", ind.ids);
    if (store.ids)
        q = q.in("store_id", store.ids);
    if (typeof data.weekday === "number")
        q = q.eq("weekday", data.weekday);
    const { data: rows, error } = await q.limit(20000);
    if (error)
        throw new Error(error.message);
    return (rows ?? [])
        .filter((r) => !ufs.ids || (r.store?.uf && ufs.ids.includes(r.store.uf)))
        .map((r) => ({
        id: r.id,
        weekday: r.weekday,
...
export const mk9RoutesListHistory = createServerFn({ method: "POST" })
    ...
    const { data: rows, error } = await supabaseAdmin
        .from("mk9_planned_routes")
        .select("id, weekday, valid_from, valid_until, is_active, archived_at, promoter:mk9_promoters(id,name)")
...
export const mk9RoutesUpsertItem = createServerFn({ method: "POST" })
    ...
    if (data.id) {
        ...
        if (prev.valid_from >= validFrom) {
            throw new Error(`A nova vigência (${validFrom}) precisa ser posterior à vigência atual (${prev.valid_from}).`);
        }
    ...
    if (iErr) {
        ...
        const match = msg.match(/MK9_ROUTE_OVERLAP\s+conflict_id=([\w-]+)\s+conflict_promoter=([\w-]+)\s+conflict_from=(\S+)\s+conflict_until=(\S+)/);
        if (match) {
            ...
            const { data: prow } = await supabaseAdmin
                ...
                .eq("id", cPromoter)
                .maybeSingle();
            throw new Error(`CONFLITO_VIGENCIA::${JSON.stringify({
                conflictRouteId: cId,
    ...
    return { id: inserted?.id, validFrom };
});
// ---------------------------------------------------------------------------
...
export const mk9RoutesDeleteItem = createServerFn({ method: "POST" })
    ...
    if (!item)
        throw new Error("Item do roteiro não encontrado ou já removido.");
...
export const mk9RoutesSetFrequency = createServerFn({ method: "POST" })
    ...
    const ctx = await requireMk9Role(["ADMIN", "SUPERVISOR"]);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: versionId, error } = await supabaseAdmin.rpc("mk9_set_frequency_manual", {
    ...
    await logAudit(ctx, "mk9_routes.set_frequency", "mk9_industry_store_frequency_versions", versionId ?? null, {
        industryId: data.industryId,
        storeId: data.storeId,
    ...
    return { ok: true, versionId: versionId ?? null };
});
// ---------------------------------------------------------------------------
...
export const mk9RoutesResolvePromoter = createServerFn({ method: "POST" })
    ...
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: rows, error } = await supabaseAdmin.rpc("mk9_resolve_route_promoter", {
        _store_id: data.storeId,
        _industry_id: data.industryId,
    ...
    if (!list.length) {
        return { status: "UNASSIGNED_ROUTE", weekdayRealized, candidates: [] };
    }
...
export const mk9StoresSearch = createServerFn({ method: "POST" })
    .inputValidator((data) => z.object({ q: z.string().min(2).max(80) }).parse(data))
    .handler(async ({ data }) => {
    const { requireMk9ReadScope } = await import("@/lib/mk9-auth/read-guards.server");
    const { scope } = await requireMk9ReadScope();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
...
export const mk9StoreGet = createServerFn({ method: "POST" })
    ...
    const { scope } = await requireMk9ReadScope();
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    if (scope.allowedStoreIds && !scope.allowedStoreIds.includes(data.id))
        return null;
